...
DOCKER_BUILD_ARGS=""
if [ "$REBUILD_IMAGE" == "--rebuild" ] || [ "$REBUILD_IMAGE" == "-r" ]; then
    echo -e "${YELLOW}Forcing Docker image rebuild (--no-cache)${NC}"
...
if [ "$PLATFORM" == "windows" ]; then
    ...
    docker build $DOCKER_BUILD_ARGS -f Dockerfile.windows -t focuslock-windows-builder .
    ...
    echo -e "${BLUE}Running Tauri build in container...${NC}"
    ...
    # Build Docker image
    echo -e "${BLUE}Building Docker image (this may take a while on first run)...${NC}"
    docker build $DOCKER_BUILD_ARGS -f Dockerfile -t focuslock-linux-builder .
    ...
    echo ""
    echo "Usage: ./build.sh [linux|windows] [--rebuild]"
    echo ""
    echo "Options:"
    echo "  linux     Build Linux packages (default)"
    echo "  windows   Build Windows installer"
    echo "  --rebuild Force Docker image rebuild (use after Dockerfile changes)"
    echo "  -r        Short form of --rebuild"
    exit 1
fi
